...
class Login extends Component {
  ...
  handleSignInClick(evnt){
    ...
    const password = formElem.querySelector("#password").value;
      ...
      if(!username || !password ){
        document.querySelector("#err").innerHTML ="Username/Password did not match";
        return;
      }
    ...
    // call Bacend api user created api
  ...
  handleSignUpClick(evnt) {
    ...
    fetch("http://localhost:5000/api/v1/user", {
      ...
      body: JSON.stringify({
        username, email, address, title, skills, job_type, fullname, password
      })
    })
      .then((resp) => resp.json())
      .then((data) => {
        if (data.error){
          ...
          document.querySelector("#err").innerHTML = "Sign up Succesfull";
        }
      })
      .catch((err) => {
        document.querySelector("#err").innerHTML = err.message;
